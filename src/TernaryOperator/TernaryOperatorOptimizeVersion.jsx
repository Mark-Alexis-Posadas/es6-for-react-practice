...
export default function TernaryOperatorOptimizeVersion() {
  const [isBool] = useState(true);
  const [image, setImage] = useState(true);
  return (
    <>
      <h1>{isBool ? "Hello World!" : "Goodbye, World!"}</h1>
      <h1>{!isBool ? "Hello World!" : "Goodbye, World!"}</h1>
      <h1 style={{ color: isBool ? "blue" : "red" }}>Hello color!</h1>
      <h1 style={{ color: !isBool ? "blue" : "red" }}>Hello color!</h1>
      ...
      <h1 style={{ fontSize: isBool ? "120px" : "5px" }}>Bwakag</h1>
      <h1 style={{ fontSize: !isBool ? "120px" : "15px" }}>Bwakag</h1>
      ...
      <button>{isBool ? "Sun" : "Moon"}</button>

      <button>{!isBool ? "Sun" : "Moon"}</button>

      <ul>{isBool ? <h1>hello</h1> : <input type="text" />}</ul>
      <ul>{!isBool ? <h1>hello</h1> : <input type="text" />}</ul>
    </>
  );
}
